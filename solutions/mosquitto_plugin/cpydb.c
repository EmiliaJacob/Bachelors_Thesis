...
//  static struct timeval oldVal = NULL;
//  static int fcounter;
//  if(oldVal == NULL){
//    oldVal = gettimeofday(&oldVal, NULL);
//  }
//  struct timeval nVal = gettimeofday(&oldVal, NULL);
//  if(nVal.tv_sec = oldVal.tv_sec == 1) {
//    fcounter += 1;
//    printf("%d\n", fcounter);
//  }
//  oldVal = nVal;
...
int mosquitto_plugin_init(mosquitto_plugin_id_t *identifier, void **user_data, struct mosquitto_opt *opts, int opt_count)
{
	...
	*user_data = mosquitto_malloc(sizeof(userdata_text));
	...
	int result = mosquitto_callback_register(mosq_pid, MOSQ_EVT_BASIC_AUTH, callback_basic_auth, NULL, *user_data)
